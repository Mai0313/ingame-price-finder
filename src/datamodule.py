...
class DataBaseManager(BaseModel):
    ...
    def table_names(self) -> list[str]:
        try:
            ...
            table_names_ = table_names_["name"].to_numpy().tolist()
            return table_names_
        except Exception:
            return []

    def save_table(
        self, table_name: str, data: pd.DataFrame, mode: str | None = "replace"
    ) -> pd.DataFrame:
        if not data.empty:
    ...
    def currency_rate(self) -> pd.DataFrame:
        ...
        if "currency_rates" in self.table_names:
            ...
            updated_time = data["database_updated_date"].to_numpy()[0]
    ...
    def update_ingame_price(
        self, table_name: str, target_game_id: str | None = None
    ) -> pd.DataFrame:
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
